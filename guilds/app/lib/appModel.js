...
Item.getItemByID = function(item_id,result){
    sql.query("Select * from guilds.item_info where item_id = ANY ($1)",[item_id],function (err,res) {
        if(err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Item.updateItemInformation = function (item, result) {
    ...
    sql.query("UPDATE guilds.item_info SET item_name=($1), item_desc =($2),image = ($3) WHERE item_id = ($4)",[item.item_name,item.item_desc,item.item_image,item.id])
...
Item.updateAvailability  = function (item, result) {
    ...
    sql.query("UPDATE guilds.item_info SET is_available=($1) WHERE item_id = ($2)",[item.is_available,item.id])
    Item.createItem(item, function (err,res){            
            if(err) {
                console.log("error updating item information: ", err);
                result(err, null);
...
Item.deleteItem = function (item_id) {
    sql.query("UPDATE guilds.item_info SET delete = 'T' where id = ($1)",[item_id],function (err,res){            
            if(err) {
...
Listing.createListing = function(listing,result){
    ...
    sql.query("INSERT INTO guilds.listings(item_id,time_posted,total_price,rent_amount,insurance_amount,lender_id) VALUES($1,$2,$3,$4,$5,$6) RETURNING *",[listing.item_id,listing.time_posted,listing.total_price,listing.rent_amount,listing.insurance_amount,listing.lender_id], function (err, res) {
...
Listing.getListingByID = function(listing_id,result){
    sql.query("Select * from guilds.listings where id = ANY ($1)",[listing_id],function (err,res) {
        if(err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Listing.addBorrower = function(ids,result){
    sql.query("UPDATE guilds.listings SET borrower_id = ($1) where id = ANY ($2)",[ids.user_id,ids.listing_id],function (err,res) {
        if(err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Listing.markCompleted = function(listing,result){
    sql.query("UPDATE guilds.listings SET completed = ($1), time_sold_expired = ($2) WHERE id = ANY ($3)",[listing.completed,listing.time_sold_expired,listing.listing_id],function (err,res) {
        if(err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Listing.deleteItem = function (listing_id) {
    sql.query("UPDATE guilds.listings SET delete = 'T' where id = ($1)",[listing_id],function (err,res){            
            if(err) {
                console.log("error deleting item: ", err);
                result(err, null);
            }
