...
User.online = async function (req, res) {
  try {
    ...
    const stat = await sql.query("UPDATE guilds.users SET online= 'T' WHERE id =($1)", [req[0].id]);
...
User.offline = async function (req, res) {
  try {
    console.log('Setting user online \n', req[0].id)
    const stat = await sql.query("UPDATE guilds.users SET online= 'F' WHERE id =($1)", [req[0].id]);
