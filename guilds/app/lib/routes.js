...
module.exports = function (app) {
	...
	app.post('/signup', async function (req, res) {
		...
		try{
			...
			await JSON.stringify(client.query('SELECT id FROM "login" WHERE "email"=$1', [req.body.username], function(err, result) {
				if(result.rows[0]){
					req.flash('warning', "This email address is already registered.");
					res.redirect('/signup');
				}
				else{
					client.query('INSERT INTO login (email,password) VALUES ($1,$2)', [req.body.username,pwd], function(err, result) {
						if(err){console.log(err);}
						else {
							client.query('INSERT INTO users (first_name,last_name,email) VALUES ($1,$2,$3)', [req.body.firstName,req.body.lastName,req.body.username])								
	...
	app.get('/login', function (req, res, next) {
		console.log('checking login get req',req)
		if (req.isAuthenticated()) {
	...
	app.post('/login',	passport.authenticate('local', {
		...
		console.log('checking login post req')
